...
class App extends Component {
  ...
  openAlbum(albumId) {
    this.setState({ viewAlbumClicked: true });
    // albumId = albumId + 1;
  ...
  state = {
    searchList: [],
    pageNumber: 1,
    viewAlbumClicked: false
  };

  render() {
    if (this.state.viewAlbumClicked === false) {
      return (
        <div>
      ...
      return (
        <div>
          <Navbar createNewSearchList={this.createNewSearchList} />
          {this.albumPhotos.map(photoObject => {
            return (
              <div className="photo-card">
                <p>{photoObject.title}</p>
                <img className="album-photo" src={photoObject.url} alt=""></img>
              </div>
            );
          })}
        </div>
